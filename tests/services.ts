...
// Create a transactional database proxy
const db = createTransactionalDatabaseProxy("default") as PgDatabase<
  any,
  any
> & {
  isTransacting: boolean;
  baseDatabase: PgDatabase<any, any>;
};

/**
 * User service with transactional methods
 */
...
/**
 * Transaction isolation testing service
 */
export class IsolationTestService {
  ...
  async createUserWithIsolationLevel(
    ...
    const txnResult = await baseDb.execute(
      "SELECT pg_current_xact_id_if_assigned() as txn_id"
    );
    const transactionId = txnResult.rows[0]?.txn_id || null;
  ...
  async getTransactionId(): Promise<string | null> {
    try {
      ...
      return txnResult.rows[0]?.txn_id || null;
    } catch (error) {
  ...
  async getRawUserCount(): Promise<number> {
    ...
    const baseDb = (db as any).baseDatabase;
    const result = await baseDb.execute("SELECT COUNT(*) as count FROM users");
